...
function getCoverURL {
	...
	url=$(echo "$url_raw" | sed -e 's/\"thumbnail_url":"//g' -e 's/"//g' -e "s/cover/$cover_size/g" -e sed 's/\\//g')
...
function getSpotifyIndex {
	...
	while read line; do
		...
		elif [[ "$line" == *"application.name ="*"spotify"* ]]; then
...
function recordPlaylist {
	...
	#echo $(getSpotifyIndex)
	pactl load-module module-null-sink sink_name=spotify
	pactl move-sink-input $(getSpotifyIndex) spotify
	##spotify.monitor
	...
	do
		...
		then
			...
			xterm -e "yes | avconv -f pulse -ac 2 -ar 48000 -i spotify.monitor -acodec flac -metadata title=\"$title\" -metadata artist=\"$artist\" -metadata album=\"$album\" -metadata track=\"$tracknumber\" -metadata disc=\"$discnumber\" \"$filename_flac\"" &
...
function monitorPlaylistFile {
	...
	while true; do
	  change=$(inotifywait -e modify,close,moved_to,create .)
	  change=${change#./ * }
